...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .banner{
    background:linear-gradient(135deg, #1F4D3F 0%, #13161A 100%);
    color:#F2EEE5;
    padding:24px 28px;
    margin-bottom:24px;
    text-align:center;
  ...
  .checks label{display:inline-block;}
  .checks .pill{
  ...
  .checks input:checked+.pill{
    background:var(--ink);
    color:var(--paper);
    border-color:var(--ink);
  }
  .scale{display:grid;grid-template-columns:repeat(5,1fr);gap:8px;margin-top:8px;}
  .scale label{position:relative;cursor:pointer;text-align:center;}
  ...
  .scale input:checked+.opt .num{color:var(--paper);}
  .footer{
    margin-top:48px;
    padding-top:24px;
    border-top:1px solid var(--rule-2);
...
<div class="wrap">
  ...
  <div class="footer">
    <p>© MINEX MEDIA · This is a read-only view of the submission</p>
  </div>
...
function getVisualStyleDisplay(value) {
  ...
  return styles.map(s => \`<label><input type="checkbox" checked disabled><span class="pill">\${s}</span></label>\`).join('');
}
